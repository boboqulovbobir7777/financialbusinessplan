  ...
  return (
    <section className="mb-25" id="resource">
      <Wrapper className="text-center">
        <h3 className="text-3xl font-bold mb-10 text-orange-500">Qushimcha Manbalar</h3>
        <ul className="flex justify-around gap-5 flex-wrap">
          {qushimcha.map((e, i) => (
            <li
              key={i}
              className="w-[180px] p-2 rounded-2xl duration-300 hover:-translate-y-1 hover:shadow-[0px_4px_5px_#791e94] text-[#791e94]"
            >
              <a
                href={e.url}
                target="_blank"
              ...
              >
                ...
                <span className="font-bold text-lg/[100%]">{e.name}</span>
              </a>
            </li>
          ))}
        </ul>
